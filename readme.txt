...
***********************************************************
    ...
    installation
        ...
        get mongo db or try free->server(community)->download zip
        its has 2 files, move unzipped folder to to required place
        open cmd

        chdir->print present working directory
        dir->list all files in pwd

        >cd mongodb

        >mongodb/bin/mongod.exe --dbpath = "C:\Users\Imam Hulagur\Desktop\JavascriptWorld\Node JS\mongodb-data"
            waiting for port->27017(default port)
        cross verify ->go to that mongodb-data folder, you will see much more files now.

        "Waiting for connections","attr":{"port":27017,"ssl":"off"}

        leave that up and running

        Installing Database GUI Viewer
        ------------------------------
        Robo 3t - >mongo admin tool
            install robot 3t(studio 3t more advanced features) and unzip
            click on exe and install it
        create->Local MongoDB Database
        click->test>gree check marks->save

        right click on connection->open shell
            db.version()->play button o/p->4.4.4

        now both mongo DB and robo 3t is un and running correctly.
    
    Connecting and Inserting Documents
    ----------------------------------
    connect mongo db from node JS application and insert documents
    we need one driver

    mongodb.com->docs->servers->documentation dropdown->drivers->Node JS->API documentation

    download mongodb npm package
        https://www.npmjs.com/package/mongodb

    create new project in code editor->task-manager
    *dont terminate running db, open new terminal!
        >cd task-manager
        >npm init -y
        >npm i mongodb(driver)

    create mongodb.js for all our task-manager CRUD operations

    Connecting mango db and MongoClient
        ...
        testing
        o/p - >Connected
        you can go to mongo to running terminal, it saying "Connection accepted"

        inserting of document
        ...
        now go to robo 3t and refresh you will see the field
        extra _id field is is unique generated by mongo to keep track of versioning
            In robo 3t we can see then collection in 3 diff formats 
            1.tree mode
            2.table mode
            3.text mode
    ...
    The ObjectID/GU ids
    -------------------
    there is no chance of 'id collision' line in SQL since server will auto incrementing integers i.e we can have user with id 1 in one db and user with same id 1 number may exist in other database.
    
    Generate some ObjectID
        const objectId = mongodb.ObjectID
        //generate id
        const id = new ObjectID()
        console.log(id)
        >node mongodb.js 
            6044d8868e5d2927b43128b0
        console.log(id)
    ...
    -----------------------
        db.collection('users').findOne({name: 'imam'}, (error, user) =>{
            if(error) {
        ...
        fetch many
        ----------
        find(), findOne(), findMany() all are diff
            find() will return cursor
            we can call toArray() from there we can take our call back.
        cursor
        ------
            The advantage of cursor
            1.sometimes i want multiple records to fetch
                 db.collection('users').find({ age: 26}).toArray((error, users)=>{
                    console.log(users)
                })
            2.sometimes i just want count, only to get single number back.
                db.collection('users').find({ age: 26}).count((error, count)=>{
                    console.log(count)
                })


        toArray() -> returns all the document
        count()-> return count of document

    Promises and its advantages
    ---------------------------
        make us to manage asynchronous code
        replaces call back, since call back is more prone to error(since it has one function to manage both cases)
        easy to understand
            ...
            here we have single function, its upto developers. wether or not error occurred and running correct code using if() condition!

            here we cant be able to call callback function twice ->program will crash!

            Promises
            --------
            mongodb has built in support for 'Promises'
            ...
            here we got 2 separate function for both cases

            here we can call both function reject(with error msg) and resolve(with data)->program wont crash.

    Updating Documents using Promises
    ---------------------------------
        update->deprecated dont use
        updateOne->allow us to update collection
        ---------
            operator $set: {} user to update
        $set: {} ->change name from 'vikram' yo 'vikrant'
            //storing promise and the listening
